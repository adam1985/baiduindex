...
var interfaceMerge = function(cb){
...
var createWorker = function(appPath){
    ...
    worker.on('message', function(res){
        if( res.complete  ){
            var processIndex = appPath.args[len-1];
            taskState[processIndex] = 1;
            appLoger('任务已完成:' + processIndex, appPath.args);
            delete workers[worker.pid];
            var workerNum = Object.keys( workers ).length;
            completeProcess.push(processIndex);
            if( completeProcess.length >= taskAmount ) {
                interfaceMerge( function(){
                    appLoger('app主程序已退出!');
                    console.log('app主程序已退出!');
                    var platform = os.platform();
                    if( !/win/.test(platform) ) {
                        var tarPath = __dirname + '/create/';
                        ...
                        tarSpawn.on('exit', function () {
                            spawn('mail', ["-s", "百度指数抓取数据", "-a" , tarPath + "baiduindex.tar.bz2",
                                'pangyuanxing@baofeng.com', 'pangyuanxing@qq.com', '<' + __dirname + '/conf/mailbody.txt'] );
